...
bool QKeyMapper_Worker::s_vbutton_panel_defaultshow = VBTNPANEL_DEFAULT_DEFAULTSHOW;
...
constexpr int KEYSEQUENCE_PAUSE_STATE_NONE = 0;
...
QString normalizeRunningKeySequenceOriginalKey(const QString &originalKey)
{
    ...
    if (separatorIndex >= 0) {
        return originalKey.left(separatorIndex);
    }

    return originalKey;
}

QStringList snapshotRunningKeySequenceOriginalKeys()
{
    QMutexLocker locker(&s_RunningKeySequenceOrikeyListMutex);
    return QKeyMapper_Worker::s_runningKeySequenceOrikeyList;
}

bool isRunningKeySequenceOriginalKey(const QString &originalKey)
{
    const QString normalizedOriginalKey = normalizeRunningKeySequenceOriginalKey(originalKey);
    if (normalizedOriginalKey.isEmpty()) {
        return false;
    }

    QMutexLocker locker(&s_RunningKeySequenceOrikeyListMutex);
    return QKeyMapper_Worker::s_runningKeySequenceOrikeyList.contains(normalizedOriginalKey);
}

bool addRunningKeySequenceOriginalKey(const QString &originalKey, QStringList *runningKeysSnapshot = Q_NULLPTR)
{
    const QString normalizedOriginalKey = normalizeRunningKeySequenceOriginalKey(originalKey);
    if (normalizedOriginalKey.isEmpty()) {
        return false;
    ...
    QMutexLocker locker(&s_RunningKeySequenceOrikeyListMutex);
    if (!QKeyMapper_Worker::s_runningKeySequenceOrikeyList.contains(normalizedOriginalKey)) {
        QKeyMapper_Worker::s_runningKeySequenceOrikeyList.append(normalizedOriginalKey);
        if (runningKeysSnapshot != Q_NULLPTR) {
            *runningKeysSnapshot = QKeyMapper_Worker::s_runningKeySequenceOrikeyList;
        }
        return true;
    }
...
bool removeRunningKeySequenceOriginalKey(const QString &originalKey, QStringList *runningKeysSnapshot = Q_NULLPTR)
{
    const QString normalizedOriginalKey = normalizeRunningKeySequenceOriginalKey(originalKey);
    if (normalizedOriginalKey.isEmpty()) {
        return false;
    ...
    QMutexLocker locker(&s_RunningKeySequenceOrikeyListMutex);
    const bool removed = QKeyMapper_Worker::s_runningKeySequenceOrikeyList.removeAll(normalizedOriginalKey) > 0;
    if (runningKeysSnapshot != Q_NULLPTR) {
        *runningKeysSnapshot = QKeyMapper_Worker::s_runningKeySequenceOrikeyList;
    }
    return removed;
...
void clearRunningKeySequenceOriginalKeys()
{
    QMutexLocker locker(&s_RunningKeySequenceOrikeyListMutex);
    QKeyMapper_Worker::s_runningKeySequenceOrikeyList.clear();
}

bool shouldPauseBeforeKeySequenceStep(const QString &originalKey, int keyupdown)
{
    if (keyupdown != KEY_DOWN) {
        return false;
    }

    static const QString keySequenceMarker = QString(":%1").arg(KEYSEQUENCE_STR);
    return originalKey.contains(keySequenceMarker);
}

SendInputTaskController *findSendInputTaskController(const QString &originalKey)
{
    const QString normalizedOriginalKey = normalizeRunningKeySequenceOriginalKey(originalKey);
    if (normalizedOriginalKey.isEmpty()) {
        return Q_NULLPTR;
    ...
    if (!SendInputTask::s_SendInputTaskControllerMap.contains(normalizedOriginalKey)) {
        return Q_NULLPTR;
    }

    return &SendInputTask::s_SendInputTaskControllerMap[normalizedOriginalKey];
}

void clearKeySequencePauseState(SendInputTaskController *controller)
{
    if (controller != Q_NULLPTR && controller->task_pause_state != Q_NULLPTR) {
        *controller->task_pause_state = KEYSEQUENCE_PAUSE_STATE_NONE;
    }
}

bool isKeySequencePauseRequestedOrActive(SendInputTaskController *controller)
{
    return controller != Q_NULLPTR
        && controller->task_pause_state != Q_NULLPTR
        && *controller->task_pause_state != KEYSEQUENCE_PAUSE_STATE_NONE;
}

bool isKeySequencePauseActive(SendInputTaskController *controller)
{
    return controller != Q_NULLPTR
        && controller->task_pause_state != Q_NULLPTR
        && *controller->task_pause_state == KEYSEQUENCE_PAUSE_STATE_ACTIVE;
}

QStringList buildKeySequencePauseTargets(const QString &originalKey)
{
    if (originalKey.isEmpty()) {
        return snapshotRunningKeySequenceOriginalKeys();
    }

    const QString normalizedOriginalKey = normalizeRunningKeySequenceOriginalKey(originalKey);
    if (normalizedOriginalKey.isEmpty() || !isRunningKeySequenceOriginalKey(normalizedOriginalKey)) {
        return QStringList();
    }
...
void cleanupKeySequenceRuntimeState(const QString &originalKey, QList<MAP_KEYDATA> *keyMappingDataList = Q_NULLPTR, bool resendRealKey = true)
{
    ...
    QStringList runningKeysSnapshot;
    const bool removed = removeRunningKeySequenceOriginalKey(normalizedOriginalKey, &runningKeysSnapshot);
    Q_UNUSED(removed);

#ifdef DEBUG_LOGOUT_ON
    if (removed) {
        qDebug().nospace().noquote() << "[cleanupKeySequenceRuntimeState] Remove OriginalKey:" << normalizedOriginalKey << ", s_runningKeySequenceOrikeyList -> " << runningKeysSnapshot;
    }
#endif

    QList<MAP_KEYDATA> *targetKeyMappingDataList = keyMappingDataList;
    if (targetKeyMappingDataList == Q_NULLPTR) {
        targetKeyMappingDataList = QKeyMapper::KeyMappingDataList;
    }
    ...
    if (targetKeyMappingDataList != Q_NULLPTR) {
        ...
        if (findindex >= 0) {
            ...
            const QStringList repeat_mappingKeyList = targetKeyMappingDataList->at(findindex).Mapping_Keys;
            const QString repeat_original_key = targetKeyMappingDataList->at(findindex).Original_Key;
            if (repeat_mappingKeyList.size() > 1 && REPEAT_MODE_BYTIMES == repeat_mode) {
                QKeyMapper_Worker::s_KeySequenceRepeatCount.remove(repeat_original_key);
            }
            return;
        }
    }

    QKeyMapper_Worker::s_KeySequenceRepeatCount.remove(normalizedOriginalKey);
}

bool waitForKeySequencePauseToClear(SendInputTaskController *controller, const QString &originalKey, int keyupdown)
{
    if (controller == Q_NULLPTR
        || controller->task_pause_state == Q_NULLPTR
        || controller->task_stop_flag == Q_NULLPTR
        || controller->task_stop_mutex == Q_NULLPTR
        || controller->task_stop_condition == Q_NULLPTR
        || !shouldPauseBeforeKeySequenceStep(originalKey, keyupdown)) {
        return false;
    }

    while (*controller->task_pause_state != KEYSEQUENCE_PAUSE_STATE_NONE) {
        if (*controller->task_stop_flag != INPUTSTOP_NONE) {
            return true;
        }

        if (*controller->task_pause_state == KEYSEQUENCE_PAUSE_STATE_REQUESTED) {
            *controller->task_pause_state = KEYSEQUENCE_PAUSE_STATE_ACTIVE;
        }

        controller->task_stop_mutex->lock();
        if (*controller->task_pause_state != KEYSEQUENCE_PAUSE_STATE_NONE
            && *controller->task_stop_flag == INPUTSTOP_NONE) {
            controller->task_stop_condition->wait(controller->task_stop_mutex);
        }
        controller->task_stop_mutex->unlock();
    }

    return *controller->task_stop_flag != INPUTSTOP_NONE;
}

bool hasPausedKeySequenceTarget(const QStringList &targetOriginalKeys)
{
    for (const QString &targetOriginalKey : std::as_const(targetOriginalKeys)) {
        if (isKeySequencePauseRequestedOrActive(findSendInputTaskController(targetOriginalKey))) {
            return true;
        }
    }
...
QAtomicBool QKeyMapper_Worker::s_Mouse2vJoy_prevValid = QAtomicBool(false);
...
QQueue<FakerInputMouseButtonExtraInfo> QKeyMapper_Worker::s_FakerInputMouseButtonExtraInfoQueue;
...
int QKeyMapper_Worker::s_GamepadTouchpad2Mouse_SendMethod = SENDMAPPINGKEY_METHOD_SENDINPUT;
    ...
    m_GamepadTouchpadStateMap()
{
    ...
    qRegisterMetaType<QJoystickTouchpadEvent>("QJoystickTouchpadEvent");
    ...
    QObject::connect(instance, &QJoysticks::touchpadEvent, QKeyMapper_Hook_Proc::getInstance(), &QKeyMapper_Hook_Proc::onJoystickTouchpadEvent);
    ...
    QObject::connect(instance, &QJoysticks::touchpadEvent, this, &QKeyMapper_Worker::onJoystickTouchpadEvent);
...
void QKeyMapper_Worker::pasteWithShiftIns()
{
...
void QKeyMapper_Worker::pasteWithCtrlV()
{
    ...
    ctrlDown.ki.time = 0;
    ctrlDown.ki.dwExtraInfo = VIRTUAL_KEY_SEND_MODIFIERS;
    ...
    vDown.ki.dwFlags = 0;
    vDown.ki.time = 0;
    vDown.ki.dwExtraInfo = VIRTUAL_KEY_SEND_MODIFIERS;
    ...
    vUp.ki.time = 0;
    vUp.ki.dwExtraInfo = VIRTUAL_KEY_SEND_MODIFIERS;
    ...
    ctrlUp.ki.time = 0;
    ctrlUp.ki.dwExtraInfo = VIRTUAL_KEY_SEND_MODIFIERS;
...
void QKeyMapper_Worker::pasteText(HWND window_hwnd, const QString &text, int mode)
{
    ...
    if (window_hwnd != NULL) {
        ...
        // No valid window handle, simulate Paste key combination globally
#ifdef DEBUG_LOGOUT_ON
        qDebug() << "[pasteText] No window handle, simulating Paste key combination globally";
#endif
...
// #endif
        ...
        for(auto it = mappingKeys.crbegin(); it != mappingKeys.crend(); ++it) {
            ...
            if (sendtype == SENDTYPE_TOGGLE || sendtype == SENDTYPE_FORCE_TOGGLE) {
...
#ifdef FAKERINPUT_SUPPORT
                    FakerInputClient_sendMouseButton(key, send_keyupdown, input.mi.dwExtraInfo);
#endif
                }
...
//#endif
                ...
                if (sendmappingkeymethod == SENDMAPPINGKEY_METHOD_FAKERINPUT && !force_sendinput) {
#ifdef FAKERINPUT_SUPPORT
                    quint8 vk = vkeycode.KeyCode;
                    FakerInputClient_sendKeyboardInput(vk, vkeycode.ExtenedFlag, send_keyupdown, input.ki.dwExtraInfo);
#endif
                }
                else if (sendmappingkeymethod == SENDMAPPINGKEY_METHOD_SENDMESSAGE) {
                    if (QKeyMapper::s_CurrentMappingHWND != NULL) {
...
// #endif
            ...
            for (const QString &keyStr : std::as_const(mappingKeys)) {
                ...
                if (key.isEmpty() || key == KEY_NONE_STR) {
                ...
                else if (key == KEYSEQUENCETOGGLE_STR) {
                    toggleRunningKeySequence();
                }
                else if (key == KEYSEQUENCEPAUSE_STR) {
                    pauseRunningKeySequence();
                }
                else if (key == KEYSEQUENCECONTINUE_STR) {
                    continueRunningKeySequence();
                }
                else if (key == KEY_RECORD_TOGGLE_MAPPING_STR) {
                    ...
                    bool vjoy_toggle_supported = (sendtype == SENDTYPE_TOGGLE || sendtype == SENDTYPE_FORCE_TOGGLE);
                    ...
                    else if ((sendtype == SENDTYPE_TOGGLE || sendtype == SENDTYPE_FORCE_TOGGLE) && vjoy_toggle_supported) {
                    ...
                    if (sendmappingkeymethod == SENDMAPPINGKEY_METHOD_FAKERINPUT) {
#ifdef FAKERINPUT_SUPPORT
                        FakerInputClient_sendMouseWheel(key, input.mi.dwExtraInfo);
#endif
                    }
                    else if (sendmappingkeymethod == SENDMAPPINGKEY_METHOD_SENDMESSAGE) {
                        if (QKeyMapper::s_CurrentMappingHWND != NULL) {
                    ...
                    else if (sendtype == SENDTYPE_TOGGLE || sendtype == SENDTYPE_FORCE_TOGGLE) {
                    ...
                    input.type = INPUT_MOUSE;
                    input.mi.mouseData = vmousecode.MouseXButton;
                    int current_sendvirtualkey_state = sendvirtualkey_state;
                    if (send_keyupdown == KEY_UP
                        && (sendtype == SENDTYPE_FORCE_UP || sendtype == SENDTYPE_FORCE_TOGGLE)) {
                        current_sendvirtualkey_state = SENDVIRTUALKEY_STATE_FORCE_KEYUP_BYPASS_REALKEY;
                    }
                    bool force_sendinput = false;
                    if (sendtype == SENDTYPE_EXCLUSION) {
                        input.mi.dwExtraInfo = VIRTUAL_KEY_OVERLAY;
                        ...
                        input.mi.dwExtraInfo = VIRTUAL_KEY_SEND | current_sendvirtualkey_state;
                    }
                    if (KEY_DOWN == send_keyupdown) {
                        input.mi.dwFlags = vmousecode.MouseDownCode;
                    }
                    ...
                    int send_keyupdown = KEY_DOWN;
                    bool toggle_supported = (sendtype == SENDTYPE_TOGGLE || sendtype == SENDTYPE_FORCE_TOGGLE);
                    if (toggle_supported
                    ...
                    else if ((sendtype == SENDTYPE_TOGGLE || sendtype == SENDTYPE_FORCE_TOGGLE) && toggle_supported) {
                        bool is_pressed = pressedVirtualKeysList.contains(key);
                        send_keyupdown = is_pressed ? KEY_UP : KEY_DOWN;
#ifdef DEBUG_LOGOUT_ON
                    ...
                    if (sendtype == SENDTYPE_EXCLUSION) {
                        ...
                        input.ki.dwExtraInfo = VIRTUAL_KEY_SEND | current_sendvirtualkey_state;
                        normal_send = true;
                    }
                    input.ki.wVk = vkeycode.KeyCode;
                    input.ki.wScan = MapVirtualKey(input.ki.wVk, MAPVK_VK_TO_VSC);
...
void QKeyMapper_Worker::emit_sendInputKeysSignal_Wrapper(int rowindex, QStringList &inputKeys, int keyupdown, QString &original_key_unchanged, int sendmode, int sendvirtualkey_state, QList<MAP_KEYDATA> *keyMappingDataList)
{
    ...
    if (key_sequence_count == 1) {
        ...
        if (mappingkeys_str == KEYSEQUENCETOGGLE_STR) {
            if (keyupdown == KEY_DOWN) {
                toggleRunningKeySequence();
            }
            return;
        }
        if (mappingkeys_str == KEYSEQUENCEPAUSE_STR) {
            if (keyupdown == KEY_DOWN) {
                pauseRunningKeySequence();
            }
            return;
        }
        if (mappingkeys_str == KEYSEQUENCECONTINUE_STR) {
            if (keyupdown == KEY_DOWN) {
                continueRunningKeySequence();
            }
            return;
    ...
    if (keyupdown == KEY_DOWN || unbreakable) {
        ...
        if (iskeysequence) {
            if (isRunningKeySequenceOriginalKey(original_key)) {
...
#ifdef DEBUG_LOGOUT_ON
                            qDebug().noquote().nospace() << "\033[1;34m[emit_sendInputKeysSignal_Wrapper] Runing KeySequence contains unbreakable OriginalKey:" << original_key << ", skip " << ((keyupdown == KEY_DOWN) ? " KeyDown" : " KeyUp") << " sendInputKeys_Signal, s_runningKeySequenceOrikeyList -> " << snapshotRunningKeySequenceOriginalKeys() << "\033[0m";
#endif
                            skip_emitsignal = true;
                        }
                    }
...
#ifdef DEBUG_LOGOUT_ON
                        qDebug().noquote().nospace() << "\033[1;34m[emit_sendInputKeysSignal_Wrapper] task_stop_flag = INPUTSTOP_KEYSEQ, Runing KeySequence contains OriginalKey:" << original_key << ", s_runningKeySequenceOrikeyList -> " << snapshotRunningKeySequenceOriginalKeys() << "\033[0m";
#endif
                        // controller->task_stop_mutex->lock();
                        controller->task_threadpool->clear();
...
#ifdef DEBUG_LOGOUT_ON
                        qDebug().noquote().nospace() << "\033[1;34m[emit_sendInputKeysSignal_Wrapper] Original KeyUp break repeat by key sequence, task_stop_flag = INPUTSTOP_KEYSEQ, Runing KeySequence contains OriginalKey:" << original_key << ", s_runningKeySequenceOrikeyList -> " << snapshotRunningKeySequenceOriginalKeys() << "\033[0m";
#endif
                        // controller->task_stop_mutex->lock();
                        controller->task_threadpool->clear();
                        *controller->task_stop_flag = INPUTSTOP_KEYSEQ;
...
bool QKeyMapper_Worker::FakerInputClient_sendKeyboardInput(quint8 vkeycode, bool extendedFlag, int keyupdown, ULONG_PTR extraInfo)
{
...
bool QKeyMapper_Worker::FakerInputClient_sendMouseButton(const QString &mouseButton, int keyupdown, ULONG_PTR extraInfo)
{
    QMutexLocker locker(&s_FakerInputClient_Mutex);
    ...
    if (extraInfo != 0) {
        ...
        if (mouseButton == "Mouse-L") {
            ...
            wParam = (keyupdown == KEY_DOWN) ? WM_RBUTTONDOWN : WM_RBUTTONUP;
        } else if (mouseButton == "Mouse-M") {
            wParam = (keyupdown == KEY_DOWN) ? WM_MBUTTONDOWN : WM_MBUTTONUP;
        } else if (mouseButton == "Mouse-X1") {
            wParam = (keyupdown == KEY_DOWN) ? WM_XBUTTONDOWN : WM_XBUTTONUP;
            xbutton = XBUTTON1;
        } else if (mouseButton == "Mouse-X2") {
            wParam = (keyupdown == KEY_DOWN) ? WM_XBUTTONDOWN : WM_XBUTTONUP;
            xbutton = XBUTTON2;
...
bool QKeyMapper_Worker::FakerInputClient_sendMouseWheel(const QString &wheelDirection, ULONG_PTR extraInfo)
{
    QMutexLocker locker(&s_FakerInputClient_Mutex);
...
bool QKeyMapper_Worker::FakerInputClient_sendMouseMove(int delta_x, int delta_y, ULONG_PTR extraInfo)
{
    Q_UNUSED(extraInfo);  // Mouse move events are not tracked in hook for now
...
void QKeyMapper_Worker::clearFakerInputExtraInfoQueues(void)
{
    ...
    {
        QMutexLocker locker(&s_FakerInputKeyboardExtraInfoQueue_Mutex);
        int keyboardQueueSize = s_FakerInputKeyboardExtraInfoQueue.size();
        s_FakerInputKeyboardExtraInfoQueue.clear();
#ifdef DEBUG_LOGOUT_ON
        if (keyboardQueueSize > 0) {
            qDebug("[clearFakerInputExtraInfoQueues] Cleared keyboard ExtraInfo queue, was size: %d", keyboardQueueSize);
        }
#else
        Q_UNUSED(keyboardQueueSize);
#endif
    }

    // Clear mouse button extraInfo queue
    {
        QMutexLocker locker(&s_FakerInputMouseButtonExtraInfoQueue_Mutex);
        int mouseQueueSize = s_FakerInputMouseButtonExtraInfoQueue.size();
        s_FakerInputMouseButtonExtraInfoQueue.clear();
#ifdef DEBUG_LOGOUT_ON
        if (mouseQueueSize > 0) {
            qDebug("[clearFakerInputExtraInfoQueues] Cleared mouse button ExtraInfo queue, was size: %d", mouseQueueSize);
        }
#else
        Q_UNUSED(mouseQueueSize);
#endif
    }

    // Clear mouse wheel extraInfo queue
    {
        QMutexLocker locker(&s_FakerInputMouseWheelExtraInfoQueue_Mutex);
        int wheelQueueSize = s_FakerInputMouseWheelExtraInfoQueue.size();
        s_FakerInputMouseWheelExtraInfoQueue.clear();
#ifdef DEBUG_LOGOUT_ON
        if (wheelQueueSize > 0) {
            qDebug("[clearFakerInputExtraInfoQueues] Cleared mouse wheel ExtraInfo queue, was size: %d", wheelQueueSize);
        }
#else
        Q_UNUSED(wheelQueueSize);
#endif
    }
}

bool QKeyMapper_Worker::matchFakerInputKeyboardExtraInfo(quint8 vkeycode, int keyupdown, ULONG_PTR &outExtraInfo)
{
    QMutexLocker locker(&s_FakerInputKeyboardExtraInfoQueue_Mutex);
...
bool QKeyMapper_Worker::matchFakerInputMouseButtonExtraInfo(WPARAM wParam, WORD xbutton, ULONG_PTR &outExtraInfo)
{
    QMutexLocker locker(&s_FakerInputMouseButtonExtraInfoQueue_Mutex);
...
bool QKeyMapper_Worker::matchFakerInputMouseWheelExtraInfo(WPARAM wParam, short wheelDelta, ULONG_PTR &outExtraInfo)
{
    QMutexLocker locker(&s_FakerInputMouseWheelExtraInfoQueue_Mutex);
...
static double maxAbsValue(double first, double second, double third, double fourth)
{
    return qMax(qMax(qAbs(first), qAbs(second)), qMax(qAbs(third), qAbs(fourth)));
}
...
#ifdef GRIP_VERBOSE_LOG
    if (rawDeltaMs > FORZA_ADJUST_MAX_DELTA_MS) {
        qDebug() << "[forzaAdjustDeltaSeconds]" << "Delta clamped ->"
                 << "rawDeltaMs =" << rawDeltaMs
                 << ", clampedDeltaMs =" << FORZA_ADJUST_MAX_DELTA_MS;
    }
#endif
...
void QKeyMapper_Worker::ViGEmClient_CheckJoysticksReportData(int gamepad_index)
{
    ...
    const int keyboardSource = static_cast<int>(VJoyStickInputSource::Keyboard);
    const int mouseSource = static_cast<int>(VJoyStickInputSource::Mouse);
    const bool hasLKeys = !pressedLStickKeysList.isEmpty();
    const bool hasRKeys = !pressedRStickKeysList.isEmpty();

    if (hasLKeys) {
        QKEYMAPPER_ATOMIC_STORE_RELAXED(reportData.ls_input_source, keyboardSource);
    }
    if (hasRKeys) {
        QKEYMAPPER_ATOMIC_STORE_RELAXED(reportData.rs_input_source, keyboardSource);
    }

    const bool keepLeftFromMouse = !hasLKeys && (QKEYMAPPER_ATOMIC_LOAD_RELAXED(reportData.ls_input_source) == mouseSource);
    const bool keepRightFromMouse = !hasRKeys && (QKEYMAPPER_ATOMIC_LOAD_RELAXED(reportData.rs_input_source) == mouseSource);

    // Reset thumb values only when not controlled by mouse
    if (!keepLeftFromMouse) {
        ViGEmTarget_Report.sThumbLX = XINPUT_THUMB_RELEASE;
        ViGEmTarget_Report.sThumbLY = XINPUT_THUMB_RELEASE;
    }
    if (!keepRightFromMouse) {
...
void QKeyMapper_Worker::ViGEmClient_Mouse2JoystickUpdate(int delta_x, int delta_y, int mouse_index, int gamepad_index)
{
    ...
    if (leftJoystickUpdate || rightJoystickUpdate) {
        ...
        if (rightJoystickUpdate) {
            ...
            QKEYMAPPER_ATOMIC_STORE_RELAXED(reportData.rs_input_source, static_cast<int>(VJoyStickInputSource::Mouse));
        }
...
void QKeyMapper_Worker::setWorkerKeyHook()
{
    ...
    clearRunningKeySequenceOriginalKeys();
...
void QKeyMapper_Worker::processForzaFormatData(const QByteArray &forzadata)
{
    ...
    double brake_peak_slip = maxAbsValue(tire_slip_ratio_FL,
                                         tire_slip_ratio_FR,
                                         tire_slip_ratio_RL,
                                         tire_slip_ratio_RR);
    double brake_slip_metric = brake_peak_slip;
    ...
    qDebug() << "[processForzaFormatData]"
             << "brake_peak_slip =" << brake_peak_slip
...
void QKeyMapper_Worker::onJoystickTouchpadEvent(const QJoystickTouchpadEvent &e)
{
...
void QKeyMapper_Worker::checkJoystickTouchpad(const QJoystickTouchpadEvent &e)
{
    if (e.joystick == Q_NULLPTR) {
        return;
    }
...
void QKeyMapper_Worker::joystickLSHorizontalProc(const QJoystickAxisEvent &e)
{
    ...
    qreal axisMagnitude = qMin<qreal>(1.0, qAbs(curr_x));
    ...
    if (ls_Left_Heavy_Pressed) {
        // State: LEFT HEAVY
        if (!dir_Left || dist < releaseThreshold || axisMagnitude < releaseThreshold) {
            // LEFT HEAVY → RELEASED
            returnFlag = JoyStickKeysProc(keycodeString_LS_Left_withoutIndex, KEY_UP, e.joystick);
            Q_UNUSED(returnFlag);
        }
        else if (dist < pushThreshold || axisMagnitude < pushThreshold) {
        ...
        // State: LEFT RELEASED
        if (dir_Left && dist >= pushThreshold && axisMagnitude >= pushThreshold) {
            // LEFT RELEASED → LEFT HEAVY
            returnFlag = JoyStickKeysProc(keycodeString_LS_Left_withoutIndex, KEY_DOWN, e.joystick);
            Q_UNUSED(returnFlag);
        }
        else if (dir_Left && dist >= lightPushThreshold && axisMagnitude >= lightPushThreshold) {
    ...
    if (ls_Right_Heavy_Pressed) {
        // State: RIGHT HEAVY
        if (!dir_Right || dist < releaseThreshold || axisMagnitude < releaseThreshold) {
        ...
        if (dir_Right && dist >= pushThreshold && axisMagnitude >= pushThreshold) {
            // RIGHT RELEASED → RIGHT HEAVY
            returnFlag = JoyStickKeysProc(keycodeString_LS_Right_withoutIndex, KEY_DOWN, e.joystick);
            Q_UNUSED(returnFlag);
        }
        else if (dir_Right && dist >= lightPushThreshold && axisMagnitude >= lightPushThreshold) {
...
void QKeyMapper_Worker::joystickLSVerticalProc(const QJoystickAxisEvent &e)
{
    ...
    qreal axisMagnitude = qMin<qreal>(1.0, qAbs(curr_y));
    ...
    if (ls_Up_Heavy_Pressed) {
        // State: UP HEAVY
        if (!dir_Up || dist < releaseThreshold || axisMagnitude < releaseThreshold) {
            // UP HEAVY → RELEASED
        ...
        // State: UP RELEASED
        if (dir_Up && dist >= pushThreshold && axisMagnitude >= pushThreshold) {
            // UP RELEASED → UP HEAVY
            returnFlag = JoyStickKeysProc(keycodeString_LS_Up_withoutIndex, KEY_DOWN, e.joystick);
            Q_UNUSED(returnFlag);
        }
        else if (dir_Up && dist >= lightPushThreshold && axisMagnitude >= lightPushThreshold) {
    ...
    if (ls_Down_Heavy_Pressed) {
        // State: DOWN HEAVY
        if (!dir_Down || dist < releaseThreshold || axisMagnitude < releaseThreshold) {
            // DOWN HEAVY → RELEASED
        ...
        // State: DOWN RELEASED
        if (dir_Down && dist >= pushThreshold && axisMagnitude >= pushThreshold) {
            // DOWN RELEASED → DOWN HEAVY
            returnFlag = JoyStickKeysProc(keycodeString_LS_Down_withoutIndex, KEY_DOWN, e.joystick);
            Q_UNUSED(returnFlag);
        }
        else if (dir_Down && dist >= lightPushThreshold && axisMagnitude >= lightPushThreshold) {
...
void QKeyMapper_Worker::gamepadTouchpad2MouseMoveProc(const QJoystickTouchpadEvent &e, int sendmappingkeymethod, bool suppressMouseMove)
{
    ...
    GameControllerTouchpadState &touchpadState = m_GamepadTouchpadStateMap[instanceId];

    if (e.eventType == TouchpadEventDown) {
        if (!touchpadState.tracking) {
            touchpadState.tracking = true;
            touchpadState.touchpad = e.touchpad;
            touchpadState.finger = e.finger;
    ...
    if (e.eventType == TouchpadEventUp) {
        touchpadState = GameControllerTouchpadState();
        return;
    }
    ...
    const qreal deltaNormalizedX = e.x - touchpadState.lastX;
    const qreal deltaNormalizedY = e.y - touchpadState.lastY;
    touchpadState.lastX = e.x;
    touchpadState.lastY = e.y;
    ...
    const double touchpadXSpeed = QKeyMapper::getGamepadTouchpadXSpeed();
    const double touchpadYSpeed = QKeyMapper::getGamepadTouchpadYSpeed();
    const bool invertX = QKeyMapper::getGamepadTouchpadInvertXStatus();
    const bool invertY = QKeyMapper::getGamepadTouchpadInvertYStatus();

    const double screenWidth = qMax(1, GetSystemMetrics(SM_CXSCREEN));
    const double screenHeight = qMax(1, GetSystemMetrics(SM_CYSCREEN));

    int delta_x = 0;
    int delta_y = 0;

    if (touchpadXSpeed > 0.0) {
        const double moveX = (invertX ? -deltaNormalizedX : deltaNormalizedX) * screenWidth * touchpadXSpeed + touchpadState.remainderX;
        delta_x = static_cast<int>(std::trunc(moveX));
        touchpadState.remainderX = moveX - delta_x;
    }
    else {
        touchpadState.remainderX = 0.0;
    }

    if (touchpadYSpeed > 0.0) {
        const double moveY = (invertY ? -deltaNormalizedY : deltaNormalizedY) * screenHeight * touchpadYSpeed + touchpadState.remainderY;
        delta_y = static_cast<int>(std::trunc(moveY));
        touchpadState.remainderY = moveY - delta_y;
    }
    else {
        touchpadState.remainderY = 0.0;
    }
...
#ifdef JOYSTICK_VERBOSE_LOG
    qDebug().nospace() << "[gamepadTouchpad2MouseMoveProc]"
                       << "P[" << e.joystick->playerindex << "] Delta X ->" << delta_x << ", Delta Y ->" << delta_y;
#endif
...
//#endif
    ...
    if ((false == keycodeString.isEmpty())
        ...
        if (extraInfo == 0) {
            ...
            if (matchFakerInputKeyboardExtraInfo(vkeycode.KeyCode, keyupdown, matchedExtraInfo)) {
                // Update extraInfo_nochanged as well for consistency
                extraInfo_nochanged = matchedExtraInfo;
                extraInfo = extraInfo_nochanged & ~0xF;
                isFakerInput = true;
#ifdef DEBUG_LOGOUT_ON
                qDebug("[LowLevelKeyboardHookProc] FakerInput matched: VK:0x%02X, %s, ExtraInfo set to 0x%08X",
                       vkeycode.KeyCode, (keyupdown == KEY_DOWN) ? "DOWN" : "UP", (unsigned int)extraInfo);
#endif
            }
        }
#endif // FAKERINPUT_SUPPORT && FAKERINPUT_EXTRAINFO
...
//                         return (LRESULT)TRUE;
            ...
            /* KEY_UP == keyupdown */
                ...
                if (HOOKPROC_STATE_STARTED != s_AtomicHookProcState
                    ...
                    && SENDVIRTUALKEY_STATE_FORCE_KEYUP_BYPASS_REALKEY != sendVirtualKeyState) {
                    return CallNextHookEx(Q_NULLPTR, nCode, wParam, lParam);
                }

                if (!isFakerInput
                    && (SENDVIRTUALKEY_STATE_NORMAL == sendVirtualKeyState
                        || SENDVIRTUALKEY_STATE_BURST_STOP == sendVirtualKeyState
                        || SENDVIRTUALKEY_STATE_FORCE == sendVirtualKeyState)) {
...
#ifdef DEBUG_LOGOUT_ON
                    QString debugmessage = QString("[LowLevelKeyboardHookProc] isFakerInput = %1, sendVirtualKeyState = %2, check RealKey \"%3\" pressed down state on keyboard is passed!").arg(isFakerInput ? "true" : "false").arg(sendVirtualKeyState).arg(keycodeString);
                    qDebug().nospace().noquote() << "\033[1;34m" << debugmessage << "\033[0m";
#endif
                }
...
LRESULT QKeyMapper_Worker::LowLevelMouseHookProc(int nCode, WPARAM wParam, LPARAM lParam)
{
    ...
    if ((wParam == WM_LBUTTONDOWN || wParam == WM_LBUTTONUP)
        ...
        if (wParam == WM_XBUTTONDOWN || wParam == WM_XBUTTONUP) {
            xbutton = GET_XBUTTON_WPARAM(mousedata);
            if ( xbutton == XBUTTON1 ) {
        ...
        if (extraInfo == 0) {
            ULONG_PTR matchedExtraInfo = 0;
            if (matchFakerInputMouseButtonExtraInfo(wParam, xbutton, matchedExtraInfo)) {
                // Update extraInfo_nochanged as well for consistency
                extraInfo_nochanged = matchedExtraInfo;
                extraInfo = extraInfo_nochanged & ~0xF;
#ifdef DEBUG_LOGOUT_ON
                qDebug("[LowLevelMouseHookProc] FakerInput matched: wParam:0x%04X, XButton:%d, ExtraInfo set to 0x%08X",
                       (unsigned int)wParam, xbutton, (unsigned int)extraInfo);
#endif
            }
        }
#endif // FAKERINPUT_SUPPORT && FAKERINPUT_EXTRAINFO
...
//                             return (LRESULT)TRUE;
        ...
        if (extraInfo == 0) {
            ULONG_PTR matchedExtraInfo = 0;
            if (matchFakerInputMouseWheelExtraInfo(wParam, zDelta, matchedExtraInfo)) {
                // Update extraInfo_nochanged as well for consistency
                extraInfo_nochanged = matchedExtraInfo;
                extraInfo = extraInfo_nochanged & ~0xF;
#ifdef DEBUG_LOGOUT_ON
                qDebug("[LowLevelMouseHookProc] FakerInput wheel matched: wParam:0x%04X, zDelta:%d, ExtraInfo set to 0x%08X",
                       (unsigned int)wParam, zDelta, (unsigned int)extraInfo);
#endif
            }
        }
#endif // FAKERINPUT_SUPPORT && FAKERINPUT_EXTRAINFO
        ...
        if (Interception_Worker::s_InterceptStart != true
            ...
            if (!s_Mouse2vJoy_EnableStateMap.isEmpty()) {
                ...
                if (s_Mouse2vJoy_Hold) {
                    ...
                    // Ensure Block-Mouse still blocks cursor movement even if hold wants passthrough.
                    if (swallowMouseMove) {
                        return (LRESULT)TRUE;
                    }
                    return CallNextHookEx(Q_NULLPTR, nCode, wParam, lParam);
                ...
                if (swallowMouseMove) {
                    returnFlag = true;
                    // Keep s_Mouse2vJoy_prev unchanged as a stable baseline.
                }
                else {
...
{
    const QStringList runningKeySequenceOriginalKeys = snapshotRunningKeySequenceOriginalKeys();
#ifdef DEBUG_LOGOUT_ON
    qDebug().nospace().noquote() << "[breakAllRunningKeySequence] Current Running KeySequence, s_runningKeySequenceOrikeyList -> " << runningKeySequenceOriginalKeys;
#endif

    for (const QString &keyseq_orikey : std::as_const(runningKeySequenceOriginalKeys)) {
        SendInputTaskController *keyseq_break_controller = findSendInputTaskController(keyseq_orikey);

        if (keyseq_break_controller != Q_NULLPTR) {
            const bool pauseActive = isKeySequencePauseActive(keyseq_break_controller);
#ifdef DEBUG_LOGOUT_ON
            ...
            qDebug().nospace().noquote() << "\033[1;34m" << debugmessage << "\033[0m";
#endif
            clearKeySequencePauseState(keyseq_break_controller);
            ...
            *keyseq_break_controller->task_stop_flag = INPUTSTOP_KEYSEQ;
            ...
            if (pauseActive) {
                cleanupKeySequenceRuntimeState(keyseq_orikey);
            }
        }
    }
}
...
void QKeyMapper_Worker::breakRunningKeySequence(const QString &originalKey)
{
    ...
    if (!isRunningKeySequenceOriginalKey(normalizedOriginalKey)) {
        // Silent no-op if target key sequence is not running.
        return;
    }

    SendInputTaskController *keyseq_break_controller = findSendInputTaskController(normalizedOriginalKey);
    ...
    const bool pauseActive = isKeySequencePauseActive(keyseq_break_controller);
#ifdef DEBUG_LOGOUT_ON
    QString debugmessage = QString("\033[1;34m[breakRunningKeySequence] OriginalKey(%1) Running KeySequence breaked, task_stop_flag = INPUTSTOP_KEYSEQ\033[0m").arg(normalizedOriginalKey);
    qDebug().nospace().noquote() << "\033[1;34m" << debugmessage << "\033[0m";
#endif
    ...
    *keyseq_break_controller->task_stop_flag = INPUTSTOP_KEYSEQ;
    ...
    if (pauseActive) {
        cleanupKeySequenceRuntimeState(normalizedOriginalKey);
    }
}

void QKeyMapper_Worker::pauseRunningKeySequence(const QString &originalKey)
{
    const QStringList pauseTargets = buildKeySequencePauseTargets(originalKey);
    for (const QString &pauseTarget : std::as_const(pauseTargets)) {
        SendInputTaskController *pauseController = findSendInputTaskController(pauseTarget);
        if (pauseController == Q_NULLPTR || pauseController->task_pause_state == Q_NULLPTR) {
            continue;
        }

        if (*pauseController->task_pause_state == KEYSEQUENCE_PAUSE_STATE_NONE) {
            *pauseController->task_pause_state = KEYSEQUENCE_PAUSE_STATE_REQUESTED;
        }
    }
}

void QKeyMapper_Worker::continueRunningKeySequence(const QString &originalKey)
{
    const QStringList continueTargets = buildKeySequencePauseTargets(originalKey);
    for (const QString &continueTarget : std::as_const(continueTargets)) {
        SendInputTaskController *continueController = findSendInputTaskController(continueTarget);
        if (continueController == Q_NULLPTR || continueController->task_pause_state == Q_NULLPTR) {
            continue;
        }

        if (*continueController->task_pause_state != KEYSEQUENCE_PAUSE_STATE_NONE) {
            *continueController->task_pause_state = KEYSEQUENCE_PAUSE_STATE_NONE;
            continueController->task_stop_condition->wakeAll();
        }
    }
}

void QKeyMapper_Worker::toggleRunningKeySequence(const QString &originalKey)
{
    const QStringList toggleTargets = buildKeySequencePauseTargets(originalKey);
    if (toggleTargets.isEmpty()) {
        return;
    }

    if (hasPausedKeySequenceTarget(toggleTargets)) {
        continueRunningKeySequence(originalKey);
    }
    else {
        pauseRunningKeySequence(originalKey);
    }
}
...
void QKeyMapper_Worker::initVirtualKeyCodeMap()
{
    ...
    VirtualKeyCodeMap.insert        (QKEYMAPPER_FN_KEY_STR,     V_KEYCODE(VK_QKEYMAPPER_FN,     EXTENED_FLAG_TRUE));   // 0x97 (QKeyMapper-Fn)
...
void QKeyMapper_Worker::initJoystickKeyMap()
{
    ...
    JoyStickKeyMap.insert("Joy-Key11(LT)_Light"         ,   (int)JOYSTICK_LEFT_TRIGGER_LIGHT);
    JoyStickKeyMap.insert("Joy-Key12(RT)_Light"         ,   (int)JOYSTICK_RIGHT_TRIGGER_LIGHT);
...
void QKeyMapper_Worker::initGlobalSendInputTaskController()
{
    ...
    controller.task_pause_state = new QAtomicInt(KEYSEQUENCE_PAUSE_STATE_NONE);
...
void QKeyMapper_Worker::clearGlobalSendInputTaskController()
{
    ...
    *controller.task_pause_state = KEYSEQUENCE_PAUSE_STATE_NONE;
    ...
    delete controller.task_pause_state;
...
void QKeyMapper_Worker::sendKeySequenceList(QStringList &keyseq_list, QString &original_key, int sendmode, int sendvirtualkey_state)
{
    ...
    for (const QString &keyseq : std::as_const(keyseq_list)){
        ...
        if (sendmode == SENDMODE_KEYSEQ_HOLDDOWN && index == size) {
            ...
            QStringList runningKeysSnapshot;
            if (addRunningKeySequenceOriginalKey(original_key, &runningKeysSnapshot)) {
#ifdef DEBUG_LOGOUT_ON
                qDebug().nospace().noquote() << "[sendKeySequenceList] Running KeySequence add OriginalKey:" << original_key << ", s_runningKeySequenceOrikeyList -> " << runningKeysSnapshot;
#endif
            }
...
void QKeyMapper_Hook_Proc::onJoystickTouchpadEvent(const QJoystickTouchpadEvent &e)
{
    QKeyMapper_Worker::getInstance()->onJoystickTouchpadEvent(e);
}
...
void SendInputTask::run()
{
    ...
    if (waitForKeySequencePauseToClear(controller, m_original_key, m_keyupdown)) {
        clearKeySequencePauseState(controller);
        *controller->task_stop_flag = INPUTSTOP_NONE;
        controller->sendvirtualkey_state = SENDVIRTUALKEY_STATE_NORMAL;
        controller->task_rowindex = INITIAL_ROW_INDEX;
        return;
